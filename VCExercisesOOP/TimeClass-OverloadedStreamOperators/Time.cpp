...
ostream&	operator<< (ostream& osObject, const Time& timeVal)				// overloaded  insertion operator << implemented as friend to Time class
{
	osObject << endl    << setfill('0')
			 << setw(2) << timeVal.hour << ':'
			 << setw(2) << timeVal.minute << ':'
			 << setw(2) << timeVal.second << endl;
...
{
	isObject >> timeVal.hour >> timeVal.minute >> timeVal.second;
...
Time::Time(int hr, int min, int sec)		// parameterized constructor
{
	setTime(hr, min, sec);
}
